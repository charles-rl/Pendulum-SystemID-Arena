...
class Critic(nn.Module):
    def __init__(self, input_dims, fc1_dims, fc2_dims, device):
        ...
        self.fc2 = layer_init(nn.Linear(fc1_dims, fc1_dims))
        self.ln_fc2 = nn.LayerNorm(fc2_dims)
        
        self.fc3 = layer_init(nn.Linear(fc2_dims, 1), std=1.0)
        self.to(device)

    def forward(self, x):
        x = self.ln_fc1(F.mish(self.fc1(x)))
...
class Actor(nn.Module):
    ...
    def forward(self, x):
        x = self.ln_fc1(F.relu(self.fc1(x)))
        x = self.ln_fc2(F.relu(self.fc2(x)))
        return F.tanh(self.fc3(x))


class AsymmetricAgent(nn.Module):
    def __init__(self, critic_input_dims, obs_dims, action_dims, device):
        super().__init__()
        self.critic = Critic(
